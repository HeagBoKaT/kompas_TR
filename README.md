...
1. **Добавление функции горячих клавиш в "Особенности":**
   - В таблицу "Особенности" добавлен пункт "Горячие клавиши" с описанием: "Настраиваемые горячие клавиши для всех действий через окно настроек."

2. **Добавление раздела "Настройка горячих клавиш":**
   - Новый раздел "Настройка горячих клавиш" описывает, как изменить горячие клавиши через окно настроек, включая инструкции по открытию окна, редактированию клавиш и примеры допустимых комбинаций.

3. **Обновление раздела "Troubleshooting":**
   - Добавлен пункт "Файл настроек устарел" с решением: удалить `settings.json`, чтобы он был пересоздан с настройками по умолчанию.

4. **Обновление раздела "Конфигурация":**
   - В разделе "Полная документация -> Конфигурация" уточнено, что файл `settings.json` теперь содержит не только настройки темы, но и горячие клавиши.
   - Добавлено описание механизма автоматической перезаписи `settings.json`, если он не соответствует текущей версии разметки.

5. **Остальные разделы:**
   - Остальные части README.md оставлены без изменений, так как они не затронуты внесенными модификациями.